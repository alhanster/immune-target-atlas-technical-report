"""
Add an `approved_drugs_targeting_nearest_gene` column to the KNN immune-target
candidate tables (both the Stim8hr and Stim48hr conditions).

For each candidate row we look up its `nearest_approved_target` gene in the
immune-system drug list and collect the approved (maxClinicalStage ==
...
NEW_COL = "approved_drugs_targeting_nearest_gene"
TARGET_COL = "nearest_approved_target"


def build_gene_to_drugs(drugs_path):
    """Map each target gene -> "; "-joined string of approved drug names."""
...
def add_column(knn_path, gene_to_drugs):
    """Add the approved-drugs column to one KNN file and write a new CSV alongside it."""
    ...
    out_path = f"{root}_with_approved_drugs{ext}"
    knn.to_csv(out_path, index=False)

    n_filled = (values != "").sum()
    print(f"Wrote {out_path}")
    print(f"  Rows: {len(knn)}  |  rows with >=1 approved drug: {n_filled}")

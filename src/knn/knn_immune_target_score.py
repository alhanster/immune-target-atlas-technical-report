#!/usr/bin/env python
"""
kNN cosine-similarity FDA drug-target score — CD4+ T-cell Perturb-seq
=====================================================================
Scores every perturbed gene by how similar its transcriptome-wide knockdown
signature is to that of known FDA-approved drug targets, using a
compare-then-aggregate k-nearest-neighbour rule:

    score(gene g) = max over known FDA targets t != g of
                    cosine( signature(g), signature(t) )

The reference set is the FDA-approved target list
(Data/Drug List/fda_approved_target_genes.txt; ~723 genes), loaded by default,
...
a deprecated *previous* symbol still match the approved FDA anchors instead of
being silently dropped.

Rationale for MAX (not mean/centroid): the FDA-target reference set is
...
OUTPUT  one CSV per condition:  knn_immune_target_candidates_<COND>.csv
  columns: gene, kNN_max_score, nearest_fda_target, is_fda_target,
...
USAGE
  ...
  python knn_immune_target_score.py --fda fda_targets.txt # custom reference list
...
# FDA reference list live under Data/ one directory up.
HERE          = Path(__file__).resolve().parent        # src/knn
REPO          = HERE.parents[1]                         # repo root
DEFAULT_FDA   = REPO / "data" / "reference" / "fda_approved_target_genes.txt"
...
def score_condition(COND, mm, cond, pert_name, ont_sig, n_cells,
    ...
    # so rows carrying a deprecated symbol still match the approved FDA anchors.
    rg_c = np.array([canon(g) for g in rg])
    pos = np.array([g in reference for g in rg_c])      # FDA-anchored positives
    pix = np.where(pos)[0]

    # compare-then-aggregate: MAX cosine to any FDA target (LOO for positives)
    ...
    return pd.DataFrame({
        "gene": rg, "kNN_max_score": sc, "nearest_fda_target": nn,
        "is_fda_target": pos, "n_downstream_z3": strength,
        "culture_condition": COND,
    }).sort_values("kNN_max_score", ascending=False).reset_index(drop=True)


def main(conditions, fda_path, refresh=False, min_cells=50):
    # Reference (anchor) set = FDA-approved targets, harmonized to current HGNC.
    # The FDA list is already 100% approved today, so harmonize is a no-op here;
    # it future-proofs against edits to the list.
    hgnc = load_hgnc()
    canon = make_canon(hgnc)
    fda_lines = [l.strip() for l in open(fda_path or DEFAULT_FDA) if l.strip()]
    reference = set(harmonize(pd.DataFrame({"gene": fda_lines}), hgnc)[0]["gene"])
    print(f"[ref] {len(reference)} FDA-approved reference genes")
    ...
    for C in conditions:
        ...
        print(f"[{C}] {len(df)} genes ({int(df.is_fda_target.sum())} FDA)  ->  {out}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--fda", default=None,

#!/usr/bin/env Rscript
# comparing_overlap_IEI_genes_and_approved.R
#
# Compares the Inborn Errors of Immunity (IEI) gene list against the targets of
# approved, immune-indicated therapeutics, and reports the overlap at both
...
#   data/reference/approved_target_genes.txt
#       Unique target genes of approved drugs with an immune indication
#       (one HGNC symbol per line).
#   Data/Drug List/Other/immune_system_drugs_approved.csv
...
#
# Usage:  Rscript comparing_overlap_IEI_genes_and_approved.R
...
approved_path   <- file.path(repo_root, "data", "reference", "approved_target_genes.txt")
drug_path  <- file.path(repo_root, "Data", "Drug List", "Other",
                        "immune_system_drugs_approved.csv")
...
approved_genes <- unique(trimws(readLines(approved_path, warn = FALSE)))
approved_genes <- approved_genes[approved_genes != ""]

# --- Gene-level overlap -----------------------------------------------------
in_both     <- sort(intersect(iei_genes, approved_genes))
iei_only    <- sort(setdiff(iei_genes, approved_genes))
approved_only    <- sort(setdiff(approved_genes, iei_genes))
union_genes <- union(iei_genes, approved_genes)

pct <- function(n, d) if (d == 0) NA_real_ else round(100 * n / d, 1)

# --- Drug-level (therapeutic) overlap ---------------------------------------
# Of the approved, immune-indicated drugs, how many target >= 1 IEI gene?
...
summary_tbl <- data.frame(
  Metric = c(
    "IEI genes (unique)",
    "approved immune-drug target genes (unique)",
    "Gene overlap (in both)",
    "IEI only",
    "approved only",
    "Union (either list)",
    "% of IEI genes that are approved immune-drug targets",
    "% of approved target genes that are IEI genes",
    "Jaccard similarity (%)",
    "approved immune-indicated drugs (unique)",
    "Drugs targeting >=1 IEI gene",
    "% of therapeutics targeting >=1 IEI gene"
  ),
  Value = c(
    length(iei_genes),
    length(approved_genes),
    length(in_both),
    length(iei_only),
    length(approved_only),
    length(union_genes),
    pct(length(in_both), length(iei_genes)),   # 14.3%
    pct(length(in_both), length(approved_genes)),   # 10.0%
...
# --- Report -----------------------------------------------------------------
cat("Overlap: IEI genes vs. approved immune-drug targets\n")
cat("=======================================================\n")
